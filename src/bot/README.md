...
5. **🧠 Промпти AI — один хаб** (`prompt.js`, команда `/prompt`, admin) — `/prompt` відкриває вибір, що редагувати: **🧠 Промпт аналізу ефективності** (guidance reduce → `app_state.analyze_prompt`, інакше `DEFAULT_REPORT_GUIDANCE`) і **⭐️ Оцінка комунікації менеджера** (критерії балу 1-10 на інжесті → `app_state.score_rubric`, інакше `DEFAULT_SCORE_RUBRIC`); далі додасться оцінка скрипту. Далі — спільний екран: 👁 переглянути / ✏️ змінити / ↩️ скинути / « Назад / « Меню. **Екрани й callback'и одні на всіх** — реєстр `EDITABLE` (`{key, button, title, about, effect, info, save, reset}`), тож нова інструкція = один запис, а стиль не може розійтися; формулювання екранів родово-нейтральні (тому й не було потреби в двох наборах речень, як у знесеного `/rubric` + `rubric.js`). ⚠️ Редагуються лише формулювання — **структуру, правило ≥3 доказів (`assembleFindings`), шкалу 1-10 і таксономію етапів (`core/stages.js`) забезпечує КОД**.
